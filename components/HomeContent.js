...
export default function HomeContent() {
    const [userCount, setUserCount] = useState(4312); // starting number

    // Simulate users connecting (for hero user count, still used)
    useEffect(() => {
        const interval = setInterval(() => {
            setUserCount((count) => count + Math.floor(Math.random() * 3)); // +0–2 users per second
        }, 1000);
        return () => clearInterval(interval);
    }, []);

    // For animated count in feature card
        ...
        <main className="bg-gradient-to-b from-indigo-100 via-white to-white min-h-screen flex flex-col scroll-smooth">
            ...
            <section className="min-h-screen flex flex-col justify-center items-center text-center px-4 relative overflow-hidden">
                ...
                <div className="z-10">
                    ...
                    {/* Live user count below buttons removed as requested */}
                </div>
                ...
                <a
                    ...
                    onClick={(e) => {
                        e.preventDefault();
                        setCount(0); // reset count to 0 to restart animation
